...
def sample_next_token_recurrent(
    ...
    logits, kv_state = model.recurrent_forward(input, step_count, kv_state)
...
def generate_recurrent(
    ...
    for i in range(len(ids)):
        ...
        next_token, kv_state = sample_next_token_recurrent(model, new_token, token_count, kv_state, T, top_p, device)
    ...
    while token_count < max_token:
        ...
        next_token, kv_state = sample_next_token_recurrent(model, next_token, token_count, kv_state, T, top_p, device)
        token_count += 1
...
def load_and_generate(
    ...
    model = RetNet(config.model_args)
